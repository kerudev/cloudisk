...
def parse() -> argparse.Namespace:
    ...
    link_parser = subparsers.add_parser(Command.LINK.value, help=f"Creates a symlink inside {CLOUDISK_ROOT}")
    link_parser.add_argument(*PATH_FLAGS, type=Path, required=True, help=(
        "If path is a file, creates a symlink to that file."
        "If path is a dir, creates a symlink for each file/dir inside itself."
    ))

    unlink_parser = subparsers.add_parser(Command.UNLINK.value, help=f"Removes a symlink inside {CLOUDISK_ROOT}")
